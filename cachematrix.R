...
makeCacheMatrix <- function(x = matrix()) {
	...
	#Set inverse matrix
	setInverse<-function(InvMatrix)Cache_Matrix<<-InvMatrix
...
cacheSolve <- function(x, ...) {
	...
	if(!is.null(Inv)) {
		...
		## gets the original matrix from the cache matrix. To check if matrix 
		##was changed or not
		Original<-solve(Inv)
		if(identical(Original,x$get()))
			return(Inv)
		else
			message("Original Matrix was changed. Recalculating inverse matrix")
